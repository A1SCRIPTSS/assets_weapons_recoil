...
local function GetAttachmentEffects(components)
    if not components or table_type(components) ~= 'table' then 
        return emptyAttachmentEffects 
    end
    
    for k in pairs(reusableAttachmentTable) do
        reusableAttachmentTable[k] = false
    end
            ...
            for attachmentType, patterns in pairs(attachmentPatterns) do
                if not reusableAttachmentTable[attachmentType] then
                    for j = 1, #patterns do
                        ...
                        if #componentLower >= pattern.len and 
                           string_find(componentLower, pattern.pattern) then
...
local function CalculateAttachmentModifiers(weaponData, attachments)
    local modifier = 1.0
    local weaponAttachments = weaponData.attachments or defaultAttachments
...
local function LogDebugChanges(weaponHash, weaponData, attachments, durabilityModifier, amplitude)
    ...
    for attachment, state in pairs(attachments) do
        if lastAttachmentState[attachment] ~= state then
            ...
            debugMsg = debugMsg .. string.format(
                "[RECOIL DEBUG] %s %s: %s (Modifier: %.2f -> %s recoil)\n",
    ...
    if durabilityModifier ~= lastDurabilityModifier then
        ...
        debugMsg = debugMsg .. string.format(
            "[RECOIL DEBUG] Durability effect %s: %.0f%% more recoil\n",
            change,
            (durabilityModifier - 1) * 100
        )
        hasChanges = true
    end
    
    if math.abs(amplitude - lastRecoilValue) > 0.01 then
        local change = amplitude > lastRecoilValue and "INCREASED" or "DECREASED"
        debugMsg = debugMsg .. string.format(
            "[RECOIL DEBUG] Recoil %s from %.2f to %.2f (%.2f%% change)\n",
            change,
            lastRecoilValue,
            amplitude,
            ((amplitude - lastRecoilValue) / lastRecoilValue) * 100
        )
        hasChanges = true
    end
    
    if hasChanges then
        if Config.Debug.detailed then
            debugMsg = debugMsg .. "[RECOIL DEBUG] Recoil Calculation Breakdown:\n"
            debugMsg = debugMsg .. string.format("- Base Recoil: %.2f\n", weaponData.recoil or 0)
            debugMsg = debugMsg .. string.format("- Weapon Type Modifier: %.2f\n", GetWeaponTypeData(weaponData.type).modifier or 1.0)
            debugMsg = debugMsg .. string.format("- Attachment Modifier: %.2f\n", CalculateAttachmentModifiers(weaponData, attachments))
            debugMsg = debugMsg .. string.format("- Durability Modifier: %.2f\n", durabilityModifier)
            debugMsg = debugMsg .. string.format("- Final Recoil Value: %.2f\n", amplitude)
        else
...
local function ApplyRecoilBasedOnStateOrSpeed(ped, weaponHash, modifier)
    ...
    local attachments = emptyAttachmentEffects

    if currentWeapon and table_type(currentWeapon) == 'table' then
        if currentWeapon.metadata and table_type(currentWeapon.metadata) == 'table' then
            if currentWeapon.metadata.components and table_type(currentWeapon.metadata.components) == 'table' then
    ...
    local amplitude = CalculateAmplitudeScale(
        pedSpeed,
        recoilSpeedMapping.minSpeed,
        recoilSpeedMapping.maxSpeed,
        recoilSpeedMapping.minEffect,
        recoilSpeedMapping.maxEffect
    ) * speedWeight

    amplitude = amplitude + CalculateAmplitudeScale(
        weaponData.recoil,
        recoilRecoilMapping.minRecoil,
        recoilRecoilMapping.maxRecoil,
        recoilRecoilMapping.minEffect,
        recoilRecoilMapping.maxEffect
    ) * recoilWeight
